...
from utils.utils_loss import partial_loss, alpha_loss, kl_loss, revised_target, out_d_loss, proden_loss, rc_loss, \
    cc_loss
from utils.utils_algo import dot_product_decode, confidence_update, confidence_update_cavl
...
parser.add_argument('-loss', type=str, choices=['proden', 'rc', 'cc', 'lws', 'cavl'])
...
def train_benchmark(config):
    ...
    for epoch in range(args.ep):
        ...
        for features, targets, trues, indexes in train_loader:
            ...
            if args.loss == 'rc' or args.loss == 'cavl':
                L_ce = rc_loss(outputs, confidence, indexes)
            elif args.loss == 'cc':
                L_ce = cc_loss(outputs, targets.float())
            confidence = confidence_update_cavl(net, confidence, features, targets, indexes)
            opt.zero_grad()
            L_ce.backward()
            opt.step()
        net.eval()
