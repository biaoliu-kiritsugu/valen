...
from utils.utils_loss import partial_loss, alpha_loss, kl_loss, revised_target, out_d_loss, proden_loss, rc_loss, \
    cc_loss, dcnn_loss
from utils.utils_algo import dot_product_decode, confidence_update
...
parser.add_argument('-loss', type=str, choices=['d2cnn'])
...
def train_benchmark(config):
    ...
    Z = torch.zeros([num_samples, num_classes])
    teacher = torch.zeros([num_samples, num_classes])
    ...
    for epoch in range(args.ep):
        ...
        if epoch < 2:
            ...
            vt = args.tmax * (np.exp(-5.0 * np.square((epoch - 2) / (200 - 2) - 1.0)))
        else:
            vt = args.tmax
        ...
        for features, targets, trues, indexes in train_loader:
            ...
            teacher_batch = teacher[indexes]
            weights_batch = weights[indexes]
            Z_batch = Z[indexes]
            Z_batch = Z_batch.to(device)

            images = Variable(features).to(device)
            labels = Variable(targets).to(device)
            teacher_batch = Variable(teacher_batch).to(device)
            weights_batch = Variable(weights_batch).to(device)
            trues = trues.to(device)

            _, output1 = net(images)
            output = F.softmax(output1, dim=1)
            loss = dcnn_loss(output, labels, teacher_batch, weights_batch, vt, args.alpha)
            ...
            teacher_batch = torch.div(Z_batch, (1 - pow(0.6, epoch + 1)))
            # for j, k in enumerate(indexes):
            Z[indexes, :] = Z_batch.detach().clone().cpu()
            teacher[indexes, :] = teacher_batch.detach().clone().cpu()

        y_partial = train_loader.dataset.train_p_Y.numpy()
        teachernp = teacher.numpy()
        label_bool = y_partial.astype(np.bool)
        ...
        weights = torch.from_numpy(np.square(margin_single))
        net.eval()
