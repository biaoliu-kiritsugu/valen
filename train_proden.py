...
from utils.utils_loss import partial_loss, alpha_loss, kl_loss, revised_target, out_d_loss, proden_loss, proden_loss_
...
parser.add_argument('-loss', type=str, default='proden', choices=['proden', 'rc', 'cc', 'lws'])
...
def train_benchmark(config):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[150, 200], last_epoch=-1)
    partial_weight = train_loader.dataset.train_p_Y.clone().detach().to(device)
    # partial_weight = train_loader.dataset.train_p_Y.to(device)
    ...
    for epoch in range(args.ep):
        ...
        for features, targets, trues, indexes in train_loader:
            ...
            L_ce, new_labels = loss_fc(outputs, partial_weight[indexes, :].clone().detach(), None)
            # L_ce, new_labels = loss_fc(outputs, confidence[indexes, :], None)
    ...
    torch.save(net.state_dict(), './cub200_256.pt')
